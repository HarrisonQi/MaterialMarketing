...
<body>
	...
	<div class="tpl-page-container tpl-page-header-fixed" style="margin-top:20px">
		<div class="">
			<div class="tpl-portlet-components">
				<div class="portlet-title">商品管理</div>
				<div id="addButtonDiv">
					<button type="button" class="am-btn am-btn-default am-btn-success" onclick="addGoods()"> 新增</button>
				</div>
				...
				<div id="MyDiv" class="white_content">
					...
					<div class="li_r" style="margin-top: 20px">
						<span>商品来源:</span><input id="addGoodsAuthor"
							style="margin-left: 70px" name="" type="text" >
					</div>
					<div class="li_r" style="margin-top: 20px">
						<span>商品介绍:</span><textarea id="addGoodsIntroduce" style="margin-left: 70px"
				...
				<div id="updateDiv" class="white_content">
					...
					<div class="li_r" style="margin-top: 20px">
						<span>商品来源:</span><input id="updateGoodsAuthor" style="margin-left: 68px"
							name="" type="text">
					</div>
					<div class="li_r" style="margin-top: 20px">
						<span>商品介绍:</span><textarea id="updateGoodsIntroduce" style="margin-left: 70px"
								rows="5" cols="65" ></textarea>
					</div>
					<div style="margin-left: 140px; margin-top: 18px">
						<button type="button"
							class="am-btn am-btn-default am-btn-secondary"
							onclick="update()">更新</button>
						<button style="margin-left: 40px" type="button"
							class="am-btn am-btn-default am-btn-danger" onclick="returnAgo()">返回</button>
					</div>
				</div>
		...
		<script>
			...
			function load() {
				$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/book/getAll',
					data : {},
					async : true,
					dataType : 'json',
						...
						if (result.code == 0) {
							for (var i = 0; i < result.data.length; i++) {
								item = "<tr><td>"
									 ...
									 + result.data[i].id
									 + ")'>删除</a><br><a href='javascript:updateGoods("
									 + result.data[i].id
									 + ")'>修改</a></td></tr>";
								 $('#table').append(item);
							}
			...
			//删除数据
			function detele(bookId){
				$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/book/detele',
					data : {
						id : bookId
					},
					async : true,
				...
				$.ajax({
					...
					success : function(result) {
						if (result.code == 0) {
							...
							$("#updateGoodsIntroduce").val(result.data.introduce);
